...
Worker.prototype.configureApplicationStack = function(app, server, callback)
{
    ...
    async.waterfall([
        ...
        // @DEPRECATED
        function(callback) {
            if (self.app.postConfigure) {
                logger.warn(
                    'Context.configure() -> app.postConfigure() was invoked\n' +
                    'app.postConfigure() will be removed with Greppy 0.8\n' +
                    'Use Context.postConfigure() instead'
                );
                return app.postConfigure(
                    app, server, callback
                );
            }
            callback && callback();
