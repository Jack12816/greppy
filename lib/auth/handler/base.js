    ...
    async.map(this.options.adapter, function(adapter, callback) {
        ...
        if ('function' !== typeof adapter.authentication) {
            return callback && callback(
                new Error('Specified adapter does not have a valie authentication method')
            );
        }

        adapter.authentication(username, credentials, function(err, isAuthenticated) {

            if (err) {
                return callback && callback(err);
            }

            callback && callback(undefined, isAuthenticated);
        });
        ...
        self.post(success, function() {
            return callback && callback(undefined, success);
        });
    });
};
